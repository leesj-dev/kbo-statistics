    ...
    for month in range(1, 13):
        ...
        for game in games:
            ...
            if game.get("roundCode") != "kbo_r":
                continue  # 시범경기/포스트시즌 제외
            if not game_id or not game_date.startswith(str(year)):
                continue
            if game_date < start_date:
                continue
            if team_code not in {game.get("homeTeamCode"), game.get("awayTeamCode")}:
                continue
            ...
            if game.get("cancel"):
                ...
                continue  # 아직 끝나지 않은 미래 경기는 제외
            elif game.get("winner") == "DRAW":
                result = "d"
            else:
                is_home_team = team_code == game.get("homeTeamCode")
                team_won = (is_home_team and game.get("winner") == "HOME") or (not is_home_team and game.get("winner") == "AWAY")
...
def generate_kbo_records(year, options="all"):
    ...
    if year not in REGULAR_SEASON_START_DATES:
        print(f"2015~2026년 사이의 데이터만 지원합니다. {year}년은 지원하지 않습니다.")
        return
...
if __name__ == "__main__":
    year = int(input("크롤링할 연도를 입력하세요 (2015~2026): "))
